...
def main():
    ...
    try:
        # 1. Extract
        raw_prices = get_bot_gold_price(URL)
        logging.info(f"Successfully fetched raw data: {raw_prices}")

        # 2. Transform
        cleaned_prices, status = clean_and_validate_prices(raw_prices)
        ...
        if status == 'OK':
            ...
            db_manager.insert_price(
                fetch_time,
                cleaned_prices["bank_selling_price"],
                cleaned_prices["bank_buying_price"],
                status
            )
